...
static int gettok() {
  ...
  if (LastChar == '0') {
	...
	if (LastChar == 'x' || LastChar == 'X') {//support hexadecimal number
	  ...
	  if (Num.length() > 0) {
	    NumVal = strtol(Num.c_str(), 0, 16);
	    return tok_number;
	  }
	}
	
 	ungetc(LastChar, stdin);// push the '9' or '8' or 'x' back
	if (ThisChar == 'x') ungetc('x', stdin);	
	LastChar = '0';
  } 
  ...
  if (LastChar == '/') {
    ...
    if (LastChar == '*') {
      LastChar = getchar();
      ThisChar = getchar();
      while (LastChar != EOF && ThisChar != EOF && (LastChar != '*' || ThisChar != '/')) {
        LastChar = ThisChar;
        ThisChar = getchar();
      }
      if (LastChar == EOF || ThisChar == EOF) return tok_eof;
      LastChar = getchar(); 
	  return gettok();
    }
    else ungetc(LastChar, stdin);
  }
